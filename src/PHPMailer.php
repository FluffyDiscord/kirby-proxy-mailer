...
class PHPMailer
{
    ...
    private function sendToProxy()
    {
        ...
        $payload = [
            ...
            'security' => $this->SMTPSecure === '' ? null : $this->SMTPSecure,
            'port' => $this->Port,

            'attachments' => []
        ];

        foreach ($this->getAttachments() as $rawAttachment) {
            // check for string attachments, and skip them
            if($rawAttachment[5] !== false) {
                continue;
            }

            $file = $rawAttachment[0];
            $contents = @file_get_contents($file);
            if($contents === false) {
                continue;
            }

            $name = $rawAttachment[2];

            $payload['attachments'][$name] = base64_encode($contents);
        }

        try {
            $options = [
                'base_uri' => env('PROXY_MAILER_ENDPOINT'),
                RequestOptions::HEADERS => [
                    'Referer' => env('BASE_URL'),
                ],
            ];

            if (env('PROXY_MAILER_AUTH')) {
                $options[RequestOptions::AUTH] = explode(':', env('PROXY_MAILER_AUTH'));
            }
            ...
            if($this->exceptions) {
                throw new \PHPMailer\PHPMailer\Exception($throwable->getMessage(), $throwable->getCode(), $throwable);
            }
            return false;
        }
